import React from "react";

function Experience() {
  return (
    <center id="experience">
      <div>
        <p className="card-title">EXPERIENCES</p>
        <br />
        <hr style={{ width: "50%" }} />
        <h4> Sales Engineer</h4>
        <h5> Sales engineer at Al-Motamakina CO. For Industrial Equipment Trading LTD - (CompAir Jordan)</h5>
        <p>CompAir Jordan | Sep 2020 - Dec 2020</p>
        <hr style={{ width: "50%" }} />
        <h4> PV Site Engineer</h4>
        <h5> "7.27 out of 46.5 MW", fixed structure system with capacity of 46.5 MW, which owned by Ministry of Energy and Mineral Resources.</h5>
        <p>Alraad Renewable Energy Est. | July 2019 - November 2019</p>
        <hr style={{ width: "50%" }} />
      </div>
      <br />
      <br />
      <br />
    </center>
  );
}

export default Experience;
